...
public class ServiceHandler {
    ...
    public init() async {
        do {
            ...
            let credConfig = Credentials(
                accessKey: "AKIAQVU25SMSKGUBDY7K",
                secret: "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",
                sessionToken: nil
...
//                credentialsProvider: AWSCredentialsProvider.fromStatic(credConfig),
                credentialsProvider: StaticCredentialsProvider(credConfig),
                region: "us-east-1"
            )
            
//            client = try S3Client(region: "us-east-1")
            client = try S3Client(config: config)
        } catch {
